...
// Full app.js with devices-to-camera integration and camera options
// ============================================
...
const State = {
  allAgriData: [],
  allSecurityData: [],
  devices: {},            // deviceId -> {name,type,module,config,linkedCameraId}
  securityCameras: {},    // camId -> {name,ip,location,active,autoDetect,addedAt}
  securityCaptures: [],
...
const Utils = {
  ...
  validateIP(ip){
    ...
    const port = (s.split('/')[0].split(':')[1])||null;
...
function showAlert(type,msg){
  if(typeof document!=='undefined' && document.getElementById){
    const c=document.getElementById('alertContainer'); if(c){ const el=document.createElement('div'); el.className='alert '+type; el.textContent=msg; c.appendChild(el); setTimeout(()=>el.remove(),5000); }
  }
  try{ EventBus.dispatchEvent(new CustomEvent('priva:alert',{detail:{type,msg}})) }catch(e){}
}

// Upload helpers (resize + xhr + retries)
...
function uploadWithXHR(url,formData,headers={},timeoutMs=30000){
  return new Promise((resolve,reject)=>{
    const xhr=new XMLHttpRequest(); let timed=false; const t=setTimeout(()=>{ timed=true; xhr.abort(); reject(new Error('timeout')) }, timeoutMs);
    xhr.open('POST',url,true); Object.entries(headers||{}).forEach(([k,v])=>{ try{xhr.setRequestHeader(k,v)}catch{} });
    xhr.onreadystatechange=()=>{ if(xhr.readyState!==4) return; clearTimeout(t); if(timed) return; if(xhr.status>=200 && xhr.status<300){ try{ resolve(JSON.parse(xhr.responseText)) }catch(e){ resolve({success:true,raw:xhr.responseText}) } } else reject(new Error('status='+xhr.status)) };
    xhr.onerror=()=>{ clearTimeout(t); reject(new Error('xhr error')) };
    try{ xhr.send(formData) }catch(e){ clearTimeout(t); reject(e) }
  });
}
async function sendBlobToAI(blob,cameraName='Téléphone',cameraId='phone_cam'){
  if(!blob) return null;
  let resized=blob; try{ resized=await resizeBlob(blob,CONFIG.RESIZE_MAX_WIDTH,'image/jpeg',CONFIG.PHONE_CAPTURE_QUALITY) }catch(e){ console.warn('resize failed',e) }
...
const AIManager = {
  ...
  async detectImage(blob, cameraName, cameraId){
    ...
    try{
      ...
      if(res && res.success && res.prediction){
        ...
        Utils.saveToStorage('priva_ai_history',this.history); this.showDetectionResult(d); this.updateAIStats(); showAlert('success',`✅ ${d.label} (${d.confidence}%)`); return d;
  ...
  async detectWithESP32(cameraIp,cameraName,cameraId){
    ...
    try{
      ...
      const r = await Utils.fetchWithTimeout(url);
      const j = await r.json();
      if(j && j.success){
        const d={cameraId,cameraName,label:j.detected||j.label,confidence:j.confidence||0,timestamp:new Date().toISOString(),source:'esp32'};
        this.history.unshift(d); if(this.history.length>50) this.history=this.history.slice(0,50);
        Utils.saveToStorage('priva_ai_history',this.history); this.showDetectionResult(d); showAlert('success',`✅ ${d.label} (${d.confidence})`); return d;
      } else showAlert('warning','Aucune détection via ESP32');
...
// Camera Manager (start/stop refresh) - uses Utils.buildCameraUrl
const CameraManager = {
  intervals:{}, fullscreenInterval:null, isActive:false, frameCounters:{},
  init(){ this.isActive=false; this.stopAll() },
  startRefresh(id, ip){
    if(!Utils.validateIP(ip)){ console.error('IP invalide',ip); return }
    this.stopRefresh(id); if(!this.isActive) return;
    let errorCount=0, MAX_ERRORS=3; this.frameCounters[id]={count:0,lastTime:Date.now()};
    const refresh=async()=>{
      if(!this.isActive){ this.stopRefresh(id); return }
      try{
        const src = Utils.buildCameraUrl(ip,'capture');
        if(typeof document!=='undefined'){
          const img=document.getElementById(`stream-${id}`); if(!img){ this.stopRefresh(id); return }
          const t=new Image(); t.onload=()=>{ img.src=src; errorCount=0; this.updateCameraStatus(id,'online'); this.updateFPS(id) }; t.onerror=()=>{ errorCount++; if(errorCount>=MAX_ERRORS){ this.updateCameraStatus(id,'offline'); this.stopRefresh(id) } }; t.src=src;
        }else{ EventBus.dispatchEvent(new CustomEvent('priva:camera-frame',{detail:{id,url:src}})); this.updateCameraStatus(id,'online'); this.updateFPS(id); }
      }catch(e){ errorCount++; if(errorCount>=MAX_ERRORS) this.stopRefresh(id) }
    };
    refresh(); this.intervals[id]=setInterval(refresh, CONFIG.CAMERA_REFRESH_RATE);
  },
  stopRefresh(id){ if(this.intervals[id]){ clearInterval(this.intervals[id]); delete this.intervals[id] } },
  stopAll(){ Object.keys(this.intervals).forEach(id=>this.stopRefresh(id)); if(this.fullscreenInterval){ clearInterval(this.fullscreenInterval); this.fullscreenInterval=null } this.frameCounters={} },
  updateCameraStatus(id,status){ if(typeof document!=='undefined'){ const ind=document.getElementById(`status-${id}`); const card=document.getElementById(`sec-cam-${id}`); const load=document.getElementById(`loading-${id}`); if(status==='online'){ ind?.classList.remove('offline'); card?.classList.remove('offline'); if(load) load.style.display='none'; } else { ind?.classList.add('offline'); card?.classList.add('offline'); if(load){ load.style.display='block'; load.textContent='❌ Hors ligne' } } } try{ EventBus.dispatchEvent(new CustomEvent('priva:camera-status',{detail:{id,status}})) }catch{} },
...
function addDeviceFromParams(name,type='sensor',module=null,config={}){
  ...
  State.devices[id]={ name:name||`Device ${id}`, type, module:module||null, config:config||{}, linkedCameraId:null, addedAt:new Date().toISOString() };
  Utils.saveToStorage('priva_devices', State.devices); renderDevices(); try{ EventBus.dispatchEvent(new CustomEvent('priva:render-devices',{detail:Object.entries(State.devices).map(([id,d])=>({id,...d}))})) }catch{}; showAlert('success',`✅ Appareil "${name}" ajouté`); return id;
}
...
function renderDevices(){
  try{ EventBus.dispatchEvent(new CustomEvent('priva:render-devices',{detail:Object.entries(State.devices).map(([id,d])=>({id,...d}))})) }catch{}
  const container = document.getElementById('devices-list'); if(!container) return;
  container.innerHTML='';
  ...
  list.forEach(([id,d])=>{
    ...
    const promoteBtn = (d.type==='camera')?`<button class="btn" id="prom_${id}">➕ Vers Sécurité</button>`:'';
    el.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;">
      <div><strong>${d.name}</strong><div class="small">${d.type} • ${d.module||'non assigné'}</div></div>
      <div style="display:flex;gap:8px;">${promoteBtn}<button class="btn" id="del_${id}">✖</button></div>
    </div>`;
    container.appendChild(el);
    if(d.type==='camera'){ document.getElementById(`prom_${id}`)?.addEventListener('click', ()=>{ promoteDeviceToCamera(id) }) }
    document.getElementById(`del_${id}`)?.addEventListener('click', ()=>{ if(confirm(`Supprimer ${d.name}?`)) removeDevice(id) });
  });
}

// Promote device -> security camera
function promoteDeviceToCamera(deviceId){
  const dev = State.devices[deviceId];
  if(!dev) return;
  // use config.ip if provided, else prompt
  const ip = dev.config?.ip || prompt('IP de la caméra (ex: 192.168.0.10 ou http://host:81):', '');
  if(!ip) { showAlert('warning','Opération annulée'); return; }
  const name = dev.name || 'Caméra';
  const loc = dev.config?.location || 'Non sp��cifié';
  const camId = 'cam_'+Date.now();
  State.securityCameras[camId] = { name, ip, location: loc, active: true, autoDetect: !!dev.config?.autoDetect, addedAt: new Date().toISOString() };
  // link device
  State.devices[deviceId].linkedCameraId = camId;
  Utils.saveToStorage('priva_security_cameras', State.securityCameras);
  Utils.saveToStorage('priva_devices', State.devices);
  renderDevices(); renderSecurityCameras();
  showAlert('success', `✅ ${name} ajouté en tant que caméra`);
}

// Security cameras render & captures
function renderSecurityCameras(){
  const list = Object.entries(State.securityCameras||{}).map(([id,c])=>({id,...c}));
  try{ EventBus.dispatchEvent(new CustomEvent('priva:render-cameras',{detail:list})) }catch{}
  const grid = document.getElementById('security-cameras-grid'); if(!grid) return;
  grid.innerHTML='';
  if(list.length===0){ grid.innerHTML='<div style="opacity:0.6">Aucune caméra configurée.</div>'; return; }
  list.forEach(cam=>{
    const card=document.createElement('div'); card.className='panel'; card.style.marginBottom='8px';
    card.innerHTML=`
      <div style="display:flex;flex-direction:column;gap:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div><strong id="cam-name-${cam.id}">${cam.name}</strong><br><span class="small" id="cam-location-${cam.id}">${cam.location}</span></div>
          <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;">
            <label class="small"><input type="checkbox" id="cam-active-${cam.id}" ${cam.active?'checked':''}/> Actif</label>
            <button class="btn" id="cam-remove-${cam.id}">🗑️</button>
          </div>
        </div>
        <img id="stream-${cam.id}" src="${Utils.buildCameraUrl(cam.ip,'capture')}" style="width:100%;height:160px;object-fit:cover;border-radius:6px;" />
        <div class="cam-settings">
          <div style="display:flex;gap:6px;">
            <input id="cam-ip-${cam.id}" value="${cam.ip||''}" style="flex:1;" />
            <button class="btn" id="cam-save-${cam.id}">💾</button>
          </div>
          <div style="display:flex;gap:6px;">
            <button class="btn" id="cam-capture-${cam.id}">📸</button>
            <button class="btn" id="cam-ia-${cam.id}">🤖</button>
            <button class="btn" id="cam-esp-${cam.id}">🔍 ESP32</button>
            <button class="btn" id="cam-flash-${cam.id}">💡 Flash</button>
            <label class="small" style="margin-left:8px;"><input type="checkbox" id="cam-autodetect-${cam.id}" ${cam.autoDetect?'checked':''}/> Auto détect</label>
          </div>
        </div>
      </div>
    `;
    ...
    document.getElementById(`cam-save-${cam.id}`)?.addEventListener('click', ()=>{
      const newIp=document.getElementById(`cam-ip-${cam.id}`).value.trim();
      const newName=document.getElementById(`cam-name-${cam.id}`).textContent.trim();
      const newLoc=document.getElementById(`cam-location-${cam.id}`).textContent.trim();
      if(!newName||!newIp){ alert('Nom et IP requis'); return; }
      State.securityCameras[cam.id].name=newName; State.securityCameras[cam.id].ip=newIp; State.securityCameras[cam.id].location=newLoc;
      Utils.saveToStorage('priva_security_cameras',State.securityCameras); showAlert('success','✅ Config sauvegardée'); renderSecurityCameras();
    });
...
function renderSecurityCaptures(){
  const gallery=document.getElementById('security-captures-gallery'); if(!gallery) return;
  gallery.innerHTML=''; if(!Array.isArray(State.securityCaptures)||State.securityCaptures.length===0){ gallery.innerHTML='<div style="opacity:0.6">Aucune capture</div>'; return; }
  State.securityCaptures.slice(0,20).forEach(cap=>{ const el=document.createElement('div'); el.className='panel'; el.innerHTML=`<img src="${cap.url}" style="width:100%;height:120px;object-fit:cover;border-radius:6px;"/><div class="small" style="margin-top:6px">${cap.name} • ${(new Date(cap.timestamp)).toLocaleTimeString()}</div>`; gallery.appendChild(el) });
}
...
async function captureCamera(id,name,ip){
  ...
  try{
    const url = Utils.buildCameraUrl(ip,'capture');
    const r = await fetch(url); const blob = await r.blob();
    const cap = { id:'cap_'+Date.now(), cameraId:id, name, timestamp:new Date().toISOString(), url };
    State.securityCaptures.unshift(cap); if(State.securityCaptures.length>CONFIG.MAX_CAPTURES) State.securityCaptures=State.securityCaptures.slice(0,CONFIG.MAX_CAPTURES);
    Utils.saveToStorage('priva_security_captures',State.securityCaptures); renderSecurityCaptures(); showAlert('success',`✅ Photo capturée: ${name}`);
    if(State.securityCameras[id]?.autoDetect || CONFIG.AI_AUTO_DETECT) await AIManager.detectImage(blob,name,id);
  }catch(e){ console.error(e); showAlert('danger','❌ Erreur capture') }
}

async function captureCameraAndDetect(id,name,ip){
  showAlert('warning','📸 Capture + Détection IA...');
  try{
    const url=Utils.buildCameraUrl(ip,'capture'); const r=await fetch(url); const blob=await r.blob();
    const cap={id:'cap_'+Date.now(),cameraId:id,name,timestamp:new Date().toISOString(),url}; State.securityCaptures.unshift(cap); if(State.securityCaptures.length>CONFIG.MAX_CAPTURES) State.securityCaptures=State.securityCaptures.slice(0,CONFIG.MAX_CAPTURES);
    Utils.saveToStorage('priva_security_captures',State.securityCaptures); renderSecurityCaptures();
    await AIManager.detectImage(blob,name,id);
  }catch(e){ console.error(e); showAlert('danger','❌ Erreur capture/détection') }
}
...
async function toggleFlash(id,ip){
  try{
    const url = Utils.buildCameraEndpoint(ip,'flash');
    ...
    try{ await Utils.fetchWithTimeout(Utils.buildCameraEndpoint(ip,'flash?state=1')); setTimeout(async()=>{ await Utils.fetchWithTimeout(Utils.buildCameraEndpoint(ip,'flash?state=0')) },200); showAlert('success','💡 Flash activé (fallback)') }catch(err){ console.error(err); showAlert('danger','❌ Erreur flash') }
  }
}

// Initialization and loading saved state
async function loadInitialState(){
  ...
  renderDevices(); renderSecurityCameras(); renderSecurityCaptures();
}
...
const PhoneCamera = {
  ...
  async start(constraints={ video:{ facingMode:{ ideal:'environment' }, width:{ ideal:1280 }, height:{ ideal:720 }}}){
    const video=document.getElementById('phone-camera-video'); if(!video){ showAlert('danger','Élément video introuvable'); return }
    if(this.stream){ try{ video.srcObject=this.stream }catch{}; showAlert('warning','Caméra déjà démarrée'); return }
    try{ const s = await navigator.mediaDevices.getUserMedia(constraints); this.stream=s; video.srcObject=s; await video.play(); showAlert('success','✅ Caméra démarrée') }catch(e){ console.error(e); showAlert('danger','Accès caméra échoué (permissions/HTTPS?)') }
  },
...
// attach functions that were defined after object (promoteDeviceToCamera etc.)
...
console.log('✅ PRIVA loaded — devices & camera module support added');
