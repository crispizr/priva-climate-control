...
const Utils = {
  ...
  saveToLocalStorage(key, data) {
    ...
    catch (error) { console.error('Erreur sauvegarde localStorage:', error); return false; }
  },
  ...
  loadFromLocalStorage(key, defaultValue = null) {
    ...
    } catch (error) { console.error('Erreur chargement localStorage:', error); return defaultValue; }
  },
  ...
  validateIP(ip) {
    ...
    return ip.split('.').every(octet => { const num = parseInt(octet, 10); return num >= 0 && num <= 255; });
  },
...
const AIManager = {
  ...
  async detectImage(imageBlob, cameraName, cameraId) {
    if (this.isProcessing) { showAlert('warning', 'Detection en cours...'); return null; }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (msg.includes('loading') || msg.includes('currently loading')) {
          throw new Error('Modele en chargement, reessayez dans 20 secondes.');
        }
        throw new Error(msg);
      }

      if (!Array.isArray(donnees) || donnees.length === 0) throw new Error('Aucun resultat retourne');
      ...
      showAlert('success', `OK: ${detection.label} (${detection.confidence}%)`);
      return detection;

    } catch (error) {
      console.error('Erreur detection IA:', error);
  ...
  updateLastResult(detection) {
    ...
    const confidenceColor = detection.confidence >= 80 ? '#00a651' : detection.confidence >= 60 ? '#f77f00' : '#e63946';
  ...
  async detectWithESP32(cameraIp, cameraName, cameraId) {
    showAlert('warning', 'Detection directe ESP32...');
    try {
      const response = await Utils.fetchWithTimeout(`http://${cameraIp}:81/detect`);
      const result = await response.json();
      if (result.success) {
        const detection = { cameraId, cameraName, label: result.detected, confidence: result.confidence, timestamp: new Date().toISOString(), source: 'esp32' };
        this.history.unshift(detection);
        this.showDetectionResult(detection);
        this.updateLastResult(detection);
        showAlert('success', `OK: ${result.detected} (${result.confidence.toFixed(1)}%)`);
        return detection;
      }
    } catch (error) {
      console.error('Erreur detection ESP32:', error);
      showAlert('danger', 'Erreur detection ESP32');
      return null;
    }
  },
  ...
  showDetectionResult(detection) {
    ...
    resultDiv.style.cssText = `background: linear-gradient(135deg, #667eea, #764ba2); padding: 15px; border-radius: 10px; margin: 10px 0; transition: all 0.3s;`;
    const confidenceColor = detection.confidence >= 80 ? '#00a651' : detection.confidence >= 60 ? '#f77f00' : '#e63946';
    resultDiv.innerHTML = `
      <div style="display: flex; align-items: center; gap: 15px;">
        ...
        <div style="flex: 1;">
          ...
          <div style="margin: 8px 0;">🎯 Detection: <strong style="font-size: 20px; color: #fff;">${detection.label}</strong></div>
          <div style="display: flex; align-items: center; gap: 10px;">
            📊 Confiance:
            <div style="flex: 1; background: rgba(255,255,255,0.2); height: 20px; border-radius: 10px; overflow: hidden;">
              <div style="width: ${detection.confidence}%; height: 100%; background: ${confidenceColor};"></div>
            </div>
            <strong style="color: ${confidenceColor};">${detection.confidence}%</strong>
          </div>
    ...
    if (container) {
      ...
      setTimeout(() => { resultDiv.style.opacity = '0'; setTimeout(() => resultDiv.remove(), 300); }, 15000);
    }
  },
  ...
  updateAIStats() {
    ...
    if (total === 0) { statsDiv.innerHTML = '<div style="text-align:center;padding:20px;opacity:0.6;">Aucune detection pour le moment</div>'; return; }
    const labels = {};
    this.history.forEach(h => labels[h.label] = (labels[h.label] || 0) + 1);
    let html = `<div style="padding:15px;background:#1a1d29;border-radius:10px;"><h3 style="margin-top:0;">📊 Statistiques IA</h3><p>Total: <strong>${total}</strong></p><div>`;
    Object.entries(labels).forEach(([label, count]) => {
      const pct = (count / total * 100).toFixed(1);
      html += `<div style="margin:5px 0;"><div style="display:flex;justify-content:space-between;"><span>${label}</span><span><strong>${count}</strong> (${pct}%)</span></div><div style="background:#0f1117;height:8px;border-radius:4px;overflow:hidden;margin-top:3px;"><div style="width:${pct}%;height:100%;background:#667eea;"></div></div></div>`;
    });
    html += `</div></div>`;
  ...
  clearHistory() {
    if (confirm('Vider l\'historique des detections IA ?')) {
      this.history = [];
      Utils.saveToLocalStorage('priva_ai_history', []);
      this.updateAIStats();
      const lastResult = document.getElementById('ai-last-result');
      if (lastResult) lastResult.style.display = 'none';
      showAlert('success', 'Historique vide');
    }
  }
};

// ==================== GESTIONNAIRE CAMERAS ====================
const CameraManager = {
  intervals: {},
  ...
  startRefresh(id, ip) {
    if (!Utils.validateIP(ip)) { console.error(`IP invalide: ${ip}`); return; }
    this.stopRefresh(id);
    ...
    const refreshFrame = async () => {
      ...
      try {
        ...
        testImg.src = newSrc;
      } catch (error) { errorCount++; if (errorCount >= MAX_ERRORS) this.stopRefresh(id); }
    };
    refreshFrame();
  ...
  startFullscreen(ip) {
    if (this.fullscreenInterval) clearInterval(this.fullscreenInterval);
    const img = document.getElementById('fullscreen-camera-img');
    if (!img) return;
    const refresh = () => {
      const modal = document.getElementById('cameraFullscreenModal');
      if (!modal || !modal.classList.contains('active')) { clearInterval(this.fullscreenInterval); this.fullscreenInterval = null; return; }
  ...
  updateCameraStatus(id, status) {
    ...
    if (status === 'online') {
      indicator?.classList.remove('offline'); card?.classList.remove('offline');
      if (loading) loading.style.display = 'none';
    } else {
      indicator?.classList.add('offline'); card?.classList.add('offline');
      if (loading) { loading.style.display = 'block'; loading.textContent = 'Hors ligne'; }
    }
  },
...
console.log('PRIVA JavaScript - Partie 1/3 chargee');

// ============================================
// PARTIE 2/3 - Init, Graphiques, Donnees, Navigation
// ============================================
...
function init() {
  ...
  showAlert('success', 'Systeme initialise');
}

// ✅ initAI EST ICI — EN DEHORS de AIManager
function initAI() {
...
function setupCharts() {
  ...
  if (climateCtx) {
    State.climateChart = new Chart(climateCtx, {
      ...
      data: { labels: [], datasets: [
        { label: 'Temperature (C)', data: [], borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', tension: 0.4, borderWidth: 3, fill: true },
        { label: 'Humidite (%)', data: [], borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', tension: 0.4, borderWidth: 3, fill: true }
      ]},
  ...
  if (airCtx) {
    State.airChart = new Chart(airCtx, {
      type: 'line',
      data: { labels: [], datasets: [
        { label: 'CO2 (ppm)', data: [], borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)', tension: 0.4, borderWidth: 3, fill: true }
      ]},
      options: chartOptions
    });
  }
...
async function loadAgricultureData() {
  ...
  } catch (err) { console.error('Erreur chargement agriculture:', err); }
}
...
function updateAgricultureTable() {
  ...
  if (recentData.length === 0) { tableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Aucune donnee</td></tr>'; return; }
  tableBody.innerHTML = recentData.map(r => `<tr><td>${r[0]}</td><td>${parseFloat(r[1]).toFixed(1)}C</td><td>${parseFloat(r[2]).toFixed(1)}%</td><td>${parseFloat(r[3]).toFixed(0)} ppm</td><td>${parseFloat(r[4]).toFixed(2)}V</td></tr>`).join('');
}
...
async function loadSecurityData() {
  ...
  } catch (err) { console.error('Erreur chargement securite:', err); }
}
...
function updateSecurityTable() {
  ...
  if (recentData.length === 0) { tableBody.innerHTML = '<tr><td colspan="6" style="text-align:center;">Aucune donnee</td></tr>'; return; }
  tableBody.innerHTML = recentData.map(r => {
    const authorized = r[4] === 'Oui';
    const authColor = authorized ? '#00a651' : '#e63946';
    return `<tr><td>${r[0]}</td><td>${r[1]||'--'}</td><td>${r[2]||'--'}</td><td>${r[3]||'--'}</td><td style="color:${authColor};font-weight:bold;">${r[4]||'--'}</td><td>${r[5]||'--'}</td></tr>`;
  }).join('');
}
...
function switchModule(module) {
  ...
  if (State.currentModule === 'security') { CameraManager.isActive = false; CameraManager.stopAll(); }
  ...
  if (module === 'agriculture' || module === 'security') {
    ...
    if (module === 'security') { setTimeout(() => { CameraManager.isActive = true; initSecurityCameras(); }, 100); }
    showAlert('success', `Module ${module === 'agriculture' ? 'Agriculture' : 'Securite'} active`);
  }
}

function showDeviceManager() {
  if (State.moduleUpdateInterval) { clearInterval(State.moduleUpdateInterval); State.moduleUpdateInterval = null; }
  CameraManager.isActive = false; CameraManager.stopAll();
  document.querySelectorAll('.module').forEach(m => m.style.display = 'none');
  const deviceManager = document.getElementById('deviceManager');
  if (deviceManager) deviceManager.style.display = 'block';
  document.querySelectorAll('.tab-btn').forEach(t => t.classList.remove('active'));
...
function updateModuleConfig(module) {
  ...
  if (!configDiv) return;
  if (!device) { configDiv.innerHTML = '<div style="text-align:center;padding:20px;opacity:0.6;">Aucun appareil actif.</div>'; return; }
  const icon = module === 'agriculture' ? '' : '';
  configDiv.innerHTML = `
    <div style="display:flex;align-items:center;gap:20px;flex-wrap:wrap;">
      <div style="flex:1;"><div style="font-weight:600;margin-bottom:5px;">${icon} ${device.name}</div><div style="font-size:12px;opacity:0.7;">${device.location}</div></div>
      <div style="flex:2;display:flex;gap:10px;align-items:center;">
        ...
        <button class="btn btn-success btn-small" onclick="updateDeviceIP('${module}')">Sauvegarder</button>
...
function updateDeviceIP(module) {
  ...
  if (!newIP || !Utils.validateIP(newIP)) { showAlert('warning', 'IP invalide'); return; }
  device.ip = newIP;
  Utils.saveToLocalStorage('priva_devices', State.devices);
  showAlert('success', `IP mise a jour: ${newIP}`);
...
async function testActiveDevice(module) {
  ...
  try { await Utils.fetchWithTimeout(`http://${device.ip}/`, { mode: 'cors' }); showAlert('success', 'Connexion reussie'); }
  catch (err) { showAlert('danger', 'Connexion echouee'); }
}

function startModuleUpdate(module) {
  const device = Object.values(State.devices).find(d => d.type === module && d.active);
  if (!device) return;
  if (module === 'agriculture') { updateAgricultureData(device.ip); State.moduleUpdateInterval = setInterval(() => updateAgricultureData(device.ip), 3000); }
  else if (module === 'security') { updateSecurityData(device.ip); State.moduleUpdateInterval = setInterval(() => updateSecurityData(device.ip), 3000); }
}
...
async function updateAgricultureData(ip) {
  ...
  } catch (err) { updateConnectionStatus('disconnected'); }
}
...
async function updateSecurityData(ip) {
  try {
    ...
    if (doorEl) { doorEl.textContent = data.doorOpen ? 'OUVERTE' : 'FERMEE'; doorEl.style.color = data.doorOpen ? '#e63946' : '#00a651'; }
    if (motionEl) { motionEl.textContent = data.motionDetected ? 'DETECTE' : 'AUCUN'; motionEl.style.color = data.motionDetected ? '#f77f00' : '#00a651'; }
    ...
    updateConnectionStatus('connected', 'Securite');
  } catch (err) { updateConnectionStatus('disconnected'); }
}
...
function updateConnectionStatus(status, module = '') {
  ...
  if (statusDot) statusDot.className = status === 'connected' ? 'status-dot connected' : 'status-dot disconnected';
  if (statusText) statusText.textContent = status === 'connected' ? `Connecte (${module})` : 'Deconnecte';
}

console.log('PRIVA JavaScript - Partie 2/3 chargee');

// ============================================
// PARTIE 3/3 - Actionneurs, Appareils, Cameras, IA
// ============================================
...
function updateDeviceUI(device, state) {
  ...
  const texts = {
    active: { pompe:'Actif', brumisateur:'Actif', ventilateur:'Actif', chauffage:'Actif', eclairage:'Allume', electrovanne:'Ouverte', lock:'Deverrouillee', alarm:'Activee', lights:'Allumees' },
    inactive: { pompe:'Arrete', brumisateur:'Arrete', ventilateur:'Arrete', chauffage:'Arrete', eclairage:'Eteint', electrovanne:'Fermee', lock:'Verrouillee', alarm:'Desactivee', lights:'Eteintes' }
  };
  if (state) { card.classList.add('active'); status.textContent = texts.active[device] || 'Actif'; }
  else { card.classList.remove('active'); status.textContent = texts.inactive[device] || 'Arrete'; }
}
...
async function toggleDevice(module, device) {
  ...
  if (activeDevice) {
    try {
      ...
      if (res.ok) { updateDeviceUI(device, newState); showAlert('success', `${device} ${newState ? 'active' : 'desactive'}`); return; }
    } catch (err) { console.error('Erreur controle local:', err); }
  }
  try {
    const response = await fetch(CONFIG.COMMAND_API_URL, { method: 'POST', headers: {'Content-Type': 'text/plain;charset=utf-8'}, body: JSON.stringify({ cible: module, actionneur: device, etat: newState ? 1 : 0 }) });
    const result = await response.json();
    if (result.status === "success") { updateDeviceUI(device, newState); showAlert('success', `${device} ${newState ? 'active' : 'desactive'} (cloud)`); }
  } catch (err) { showAlert('danger', 'Erreur commande'); }
}
...
async function setMode(mode) {
  ...
  if (activeDevice) {
    try {
      const res = await Utils.fetchWithTimeout(`http://${activeDevice.ip}/mode`, { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: `mode=${mode}` });
      if (res.ok) { const modeDisplay = document.getElementById('modeDisplay'); if (modeDisplay) modeDisplay.textContent = mode.toUpperCase(); showAlert('success', `Mode ${mode}`); return; }
    } catch (err) { console.error('Erreur setMode:', err); }
  }
  showAlert('warning', 'Connectez un ESP32');
}

async function emergencyStop() {
  if (!confirm('CONFIRMER ARRET D\'URGENCE ?')) return;
  const activeDevice = Object.values(State.devices).find(d => d.type === 'agriculture' && d.active);
  if (activeDevice) {
    try {
      ...
      showAlert('danger', 'ARRET D\'URGENCE');
      return;
    } catch (err) { console.error('Erreur emergencyStop:', err); }
  }
  showAlert('danger', 'ARRET D\'URGENCE (cloud)');
}

async function saveSettings() {
  const activeDevice = Object.values(State.devices).find(d => d.type === 'agriculture' && d.active);
  const settings = { tempMin: document.getElementById('tempMin')?.value, tempMax: document.getElementById('tempMax')?.value, humMin: document.getElementById('humidMin')?.value, humMax: document.getElementById('humidMax')?.value };
  if (activeDevice) {
    try {
      await Utils.fetchWithTimeout(`http://${activeDevice.ip}/settings`, { method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded'}, body: `tempMin=${settings.tempMin}&tempMax=${settings.tempMax}&humMin=${settings.humMin}&humMax=${settings.humMax}` });
      showAlert('success', 'Parametres enregistres'); return;
    } catch (err) { console.error('Erreur saveSettings:', err); }
  }
  showAlert('warning', 'Connectez un ESP32');
}
...
function renderDevicesList() {
  ...
  if (!list) return;
  if (Object.keys(State.devices).length === 0) { list.innerHTML = '<div style="text-align:center;padding:40px;opacity:0.6;">Aucun appareil. Cliquez sur "Ajouter"</div>'; return; }
  list.innerHTML = Object.entries(State.devices).map(([id, dev]) => {
    const icon = dev.type === 'agriculture' ? '' : dev.type === 'security' ? '' : '';
    return `<div class="device-item"><div class="device-info"><div class="device-name">${icon} ${dev.name}</div><div class="device-details">${dev.ip} - ${dev.location} - <span style="color:${dev.active ? '#00a651' : '#e63946'};font-weight:bold;">${dev.active ? 'Actif' : 'Inactif'}</span></div></div><div class="device-actions"><button class="btn btn-small ${dev.active ? 'btn-danger' : 'btn-success'}" onclick="toggleDeviceActive('${id}')">${dev.active ? 'Pause' : 'Activer'}</button><button class="btn btn-small btn-primary" onclick="testDeviceConnection('${id}')">Tester</button><button class="btn btn-small btn-danger" onclick="deleteDevice('${id}')">Supprimer</button></div></div>`;
  }).join('');
}

function openAddDeviceModal() { const modal = document.getElementById('addDeviceModal'); if (modal) modal.classList.add('active'); }
function closeAddDeviceModal() { const modal = document.getElementById('addDeviceModal'); if (modal) modal.classList.remove('active'); }
...
function addDevice() {
  ...
  if (!name || !ip) { showAlert('warning', 'Nom et IP requis'); return; }
  if (!Utils.validateIP(ip)) { showAlert('warning', 'Format IP invalide'); return; }
  const id = 'dev_' + Date.now();
  State.devices[id] = { name, ip, type, location: location || 'Non specifie', active: true, addedAt: new Date().toISOString() };
  Object.entries(State.devices).forEach(([key, dev]) => { if (key !== id && dev.type === type) dev.active = false; });
  Utils.saveToLocalStorage('priva_devices', State.devices);
  closeAddDeviceModal();
  renderDevicesList();
  showAlert('success', `${name} ajoute`);
  ['newDeviceName','newDeviceIP','newDeviceLocation'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
  if (type === 'agriculture' || type === 'security') {
    setTimeout(() => { const moduleBtn = document.querySelector(`.tab-btn[onclick*="switchModule('${type}')"]`); if (moduleBtn) moduleBtn.click(); }, 500);
  }
}
...
function toggleDeviceActive(id) {
  ...
  if (!device) return;
  if (!device.active) {
    Object.entries(State.devices).forEach(([key, dev]) => { if (dev.type === device.type) dev.active = false; });
    device.active = true;
    showAlert('success', `${device.name} active`);
  ...
  } else { device.active = false; showAlert('warning', `${device.name} desactive`); if (State.moduleUpdateInterval) clearInterval(State.moduleUpdateInterval); }
  Utils.saveToLocalStorage('priva_devices', State.devices);
  renderDevicesList();
}
...
async function testDeviceConnection(id) {
  ...
  try { await Utils.fetchWithTimeout(`http://${device.ip}/`, {mode: 'cors'}); showAlert('success', `${device.name} repond`); }
  catch (err) { showAlert('danger', `${device.name} ne repond pas`); }
}
...
function deleteDevice(id) {
  ...
  Utils.saveToLocalStorage('priva_devices', State.devices);
  renderDevicesList();
  showAlert('success', `${device.name} supprime`);
}
...
function renderSecurityCameras() {
  ...
  if (active.length === 0) { grid.innerHTML = '<div style="text-align:center;padding:40px;opacity:0.6;">Aucune camera configuree.</div>'; return; }
  grid.innerHTML = active.map(([id, cam]) => `
    <div class="security-camera-card" id="sec-cam-${id}">
      <div class="camera-header">
        <div class="camera-name">📹 ${cam.name}<div class="camera-status-indicator" id="status-${id}"></div></div>
        <button class="btn btn-small btn-danger" onclick="removeSecurityCamera('${id}')">Supprimer</button>
      </div>
      <div style="position:relative;background:#000;border-radius:8px;min-height:250px;overflow:hidden;">
        <img class="camera-stream-img" id="stream-${id}" src="${Utils.buildCameraUrl(cam.ip, 'capture')}" onclick="openCameraFullscreen('${id}','${cam.name}','${cam.ip}')" alt="${cam.name}" style="width:100%;height:100%;object-fit:cover;cursor:pointer;">
        <div id="loading-${id}" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);color:white;display:none;">Chargement...</div>
      </div>
      <div class="camera-controls">
        <button class="btn btn-small btn-success" onclick="captureCamera('${id}','${cam.name}','${cam.ip}')">Capturer</button>
        <button class="btn btn-small" onclick="captureCameraAndDetect('${id}','${cam.name}','${cam.ip}')" style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;">IA</button>
        ...
        <button class="btn btn-small btn-primary" onclick="openCameraFullscreen('${id}','${cam.name}','${cam.ip}')">Plein ecran</button>
      </div>
      <div class="camera-info">${cam.location} - ${cam.ip} - FPS: <span id="fps-${id}">--</span></div>
    </div>`).join('');
  if (CameraManager.isActive) active.forEach(([id, cam]) => CameraManager.startRefresh(id, cam.ip));
}

function renderSecurityCaptures() {
  const gallery = document.getElementById('security-captures-gallery');
  if (!gallery) return;
  if (State.securityCaptures.length === 0) { gallery.innerHTML = '<div style="text-align:center;padding:20px;opacity:0.6;">Aucune capture</div>'; return; }
  gallery.innerHTML = State.securityCaptures.slice(0, 20).map((cap, idx) => {
    const date = new Date(cap.timestamp);
    return `<div class="capture-item" onclick="viewCapture(${idx})"><img src="${cap.url}" alt="${cap.name}" loading="lazy"><div class="capture-info"><div>${cap.name}</div><div>${date.toLocaleTimeString('fr-FR')}</div></div><button class="capture-delete" onclick="event.stopPropagation();deleteCapture(${idx})">X</button></div>`;
  }).join('');
}
...
async function captureCamera(id, name, ip) {
  ...
  try {
    ...
    renderSecurityCaptures();
    showAlert('success', `Photo capturee: ${name}`);
    if (CONFIG.AI_AUTO_DETECT) await AIManager.detectImage(blob, name, id);
  } catch (error) { console.error('Erreur capture:', error); showAlert('danger', 'Erreur capture'); }
}

async function captureCameraAndDetect(id, name, ip) {
  showAlert('warning', 'Capture + Detection IA...');
  try {
    const captureUrl = Utils.buildCameraUrl(ip, 'capture');
    const response = await fetch(captureUrl);
    const blob = await response.blob();
  ...
  } catch (error) { console.error('Erreur:', error); showAlert('danger', 'Erreur capture/detection'); }
}

async function detectWithESP32Camera(id, name, ip) { await AIManager.detectWithESP32(ip, name, id); }

function captureAllCameras() {
  const active = Object.entries(State.securityCameras).filter(([id, cam]) => cam.active);
  if (active.length === 0) { showAlert('warning', 'Aucune camera active'); return; }
  active.forEach(([id, cam], idx) => setTimeout(() => captureCamera(id, cam.name, cam.ip), idx * 500));
}

async function toggleFlash(id, ip) {
  try {
    await Utils.fetchWithTimeout(`http://${ip}:${CONFIG.ESP32_PORT}/flash`, { method: 'POST', headers: {'Content-Type':'application/x-www-form-urlencoded'}, body: 'state=1' });
    setTimeout(async () => { await Utils.fetchWithTimeout(`http://${ip}:${CONFIG.ESP32_PORT}/flash`, { method: 'POST', body: 'state=0' }); }, 200);
    showAlert('success', 'Flash active');
  } catch (err) { showAlert('danger', 'Erreur flash'); }
}

function refreshAllCameras() {
  Object.entries(State.securityCameras).forEach(([id, cam]) => { const img = document.getElementById(`stream-${id}`); if (img) img.src = Utils.buildCameraUrl(cam.ip, 'capture'); });
  showAlert('success', 'Cameras rafraichies');
}
...
function openAddCameraModal() { const modal = document.getElementById('addCameraModal'); if (modal) modal.classList.add('active'); }
function closeAddCameraModal() { const modal = document.getElementById('addCameraModal'); if (modal) modal.classList.remove('active'); }
...
function addSecurityCamera() {
  ...
  State.securityCameras[id] = { name, ip, location: location || 'Non specifie', active: true, addedAt: new Date().toISOString() };
  Utils.saveToLocalStorage('priva_security_cameras', State.securityCameras);
  closeAddCameraModal();
  renderSecurityCameras();
  showAlert('success', `${name} ajoutee`);
  ['newCameraName','newCameraIP','newCameraLocation'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
}
...
function removeSecurityCamera(id) {
  ...
  Utils.saveToLocalStorage('priva_security_cameras', State.securityCameras);
  renderSecurityCameras();
  showAlert('success', `${cam.name} supprimee`);
}

function viewCapture(idx) { const cap = State.securityCaptures[idx]; if (!cap) return; openCameraFullscreen(null, cap.name, null, cap.url); }
...
function clearSecurityCaptures() { if (!confirm('Vider toutes les captures ?')) return; State.securityCaptures = []; Utils.saveToLocalStorage('priva_security_captures', State.securityCaptures); renderSecurityCaptures(); showAlert('success', 'Galerie videe'); }
...
function openCameraFullscreen(id, name, ip, captureUrl = null) {
  ...
  else if (ip) { img.src = Utils.buildCameraUrl(ip, 'capture'); CameraManager.startFullscreen(ip); }
...
function closeCameraFullscreen() {
  ...
  if (CameraManager.fullscreenInterval) { clearInterval(CameraManager.fullscreenInterval); CameraManager.fullscreenInterval = null; }
}
...
function captureFromFullscreen() {
  ...
  if (active) { const [id, cam] = active; captureCamera(id, cam.name, cam.ip); }
}
...
function toggleFlashFullscreen() {
  ...
  if (!img || !img.src) return;
  const active = Object.entries(State.securityCameras).find(([id, cam]) => img.src.includes(cam.ip));
  if (active) { const [id, cam] = active; toggleFlash(id, cam.ip); }
}
...
function downloadCapture() {
  ...
  link.click();
  showAlert('success', 'Telechargement...');
}
...
async function testAIServer() {
  ...
  try {
    ...
    if (data.status === 'healthy') { showAlert('success', 'Serveur IA operationnel'); return true; }
  } catch (error) { showAlert('danger', 'Serveur IA injoignable'); return false; }
}

async function getAIModelInfo() {
  const infoDiv = document.getElementById('ai-model-info');
  if (infoDiv) {
    infoDiv.innerHTML = `<div style="padding:10px;background:#1a1d29;border-radius:8px;font-size:12px;"><strong>Modele HF:</strong> ${CONFIG.HF_MODEL}<br><strong>Serveur Render:</strong> ${CONFIG.RENDER_URL}<br><strong>Version:</strong> v4.0</div>`;
  }
}
...
function toggleAutoDetect() {
  ...
  if (btn) { btn.textContent = CONFIG.AI_AUTO_DETECT ? 'Auto: ON' : 'Auto: OFF'; btn.className = CONFIG.AI_AUTO_DETECT ? 'btn btn-success btn-small' : 'btn btn-secondary btn-small'; }
  showAlert(CONFIG.AI_AUTO_DETECT ? 'success' : 'warning', CONFIG.AI_AUTO_DETECT ? 'Detection auto activee' : 'Detection auto desactivee');
}
...
function sauvegarderToken() {
  ...
  showAlert('success', 'Token sauvegarde !');
}
...
function chargerToken() {
  ...
  if (token) {
    ...
    console.log('Token HF charge depuis localStorage');
  }
}
...
console.log('PRIVA JavaScript Complet v4.0 Final - Toutes fonctionnalites chargees');
